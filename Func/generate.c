...
void find_MCQ(FILE* op, Paper* P, Bank* B)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        while (trav != NULL)
        {
            if (trav->no_corr <= ->no_ops &&
                trav->diff >= P->mcq_reqs[i]->diff_lb &&
...
void find_FITB(FILE* op, Paper* P, Bank* B)
{
    ...
    while (trav != NULL)
    {
        if (trav->diff >= P->fitb_reqs->diff_lb &&
            trav->diff <= P->fitb_reqs->diff_ub)
                possible[found++] = trav;
...
void find_TF(FILE* op, Paper* P, Bank* B)
{
    ...
    while (trav != NULL)
    {
        if (trav->diff >= P->tf_reqs->diff_lb &&
            trav->diff <= P->tf_reqs->diff_ub)
                possible[found++] = trav;
...
void find_NUM(FILE* op, Paper* P, Bank* B)
{
    ...
    while (trav != NULL)
    {
        if (trav->diff >= P->num_reqs->diff_lb &&
            trav->diff <= P->num_reqs->diff_ub)
                possible[found++] = trav;
